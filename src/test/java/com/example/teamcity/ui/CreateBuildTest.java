...
public class CreateBuildTest extends BaseUiTest{
    ...
    public void userCreatesBuildConfigurationWitjEmptyBuildName() {
        loginAs(testData.getUser());
        var userCheckRequests = new CheckedRequests(Specifications.authSpec(testData.getUser()));
        var project = userCheckRequests.<Project>getRequest(Endpoint.PROJECTS).create(testData.getProject());

        String errorMessage = CreateBuildConfigurationPage.open(project.getId()).createForm(REPO_URL).setBuildConfigurationName("").getBuildConfigurationNameErrorMessage();

        var foundBuilds = ProjectBuildsPage.open(project.getId()).getBuilds().stream().map(builds -> builds.getName().text()).collect(Collectors.toList());
        soft.assertTrue(foundBuilds.isEmpty());
        soft.assertEquals(errorMessage, "Build configuration name must not be empty");
    }
    ...
    public void userRunsTheBuildSucessfully() {
        ...
        soft.assertEquals(buildElement.getBuildStatus().getText(), "Success");
    }
}
